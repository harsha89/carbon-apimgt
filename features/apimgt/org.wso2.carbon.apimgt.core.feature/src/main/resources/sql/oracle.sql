...
CREATE TABLE AM_ENDPOINT (
    ...
    PRIMARY KEY(ID),
    UNIQUE (UUID),
    UNIQUE (ENDPOINT_NAME, ENDPOINT_VERSION)
)
/
-- Generate ID using sequence and trigger
CREATE SEQUENCE AM_ENDPOINT_seq START WITH 1 INCREMENT BY 1
/
CREATE OR REPLACE TRIGGER AM_ENDPOINT_seq_tr
 BEFORE INSERT ON AM_ENDPOINT FOR EACH ROW
 WHEN (NEW.ID IS NULL)
BEGIN
 SELECT AM_ENDPOINT_seq.NEXTVAL INTO :NEW.ID FROM DUAL;
END;
/
...
/
create index IDX_AET_NME on AM_ENDPOINT (NAME);
/
